...
export default {
  ...
  methods: {
    ...
    async readReporte1() {
      ...
      try {
        var datos = await this.$axios.post(process.env.baseUrl + "/reporte2", {
          token: this.token,
    ...
    async createPDFSalidasControles() 
    {
      ...
      for (var i = 0; i < this.mListaReporte1.length; i++) 
      {
        ...
        {
          totalPendiente = totalPendiente+25
        }else if(this.mListaReporte1[i].estado == 3)
        {
          totalRecaudado = totalRecaudado+25
        }
        resultadoString.push([
          ...
          {
            text: this.mListaReporte1[i].motivo,
            fontSize: 8.5,
            alignment: "center",
          },
          {
            text:
              this.mListaReporte1[i].estado == 1
                ? "EN ESPERA"
                : this.mListaReporte1[i].estado == 2
                ? "VERIFICANDO"
                : this.mListaReporte1[i].estado == 3
                ? "PAGADO"
                : "ERROR DE RECIBO",
            fontSize: 8.5,
            alignment: "center",
