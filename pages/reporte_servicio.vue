...
export default {
  ...
  methods: {
    async readReporte1() 
    {
      ...
      try {
        var datos = await this.$axios.post(process.env.baseUrl + "/reporte_servicio", {
          token: this.token,
    ...
    async createPDFSalidasControles() {
      ...
      for (var i = 0; i < this.mListaReporte1.length; i++) {
        resultadoString.push([
          {
            text: this.mListaReporte1[i].fechaReserva,
            fontSize: 8.5,
            alignment: "center",
          },
          {
            text: this.mListaReporte1[i].detalle_servicio,
            fontSize: 8.5,
            alignment: "center",
          },
          {
          ...
          {
            text: this.mListaReporte1[i].telefono_usuario,
            fontSize: 8.5,
            alignment: "center",
          },
          {
            text: this.mListaReporte1[i].horario,
            fontSize: 8.5,
            alignment: "center",
          },
          {
            text: (this.mListaReporte1[i].estado_reserva == 1 ? "EN ESPERA" : this.mListaReporte1[i].estado_reserva == 2 ? "RESERVADO" : "RECHAZADO"),
            fontSize: 8.5,
            alignment: "center",
          },
        ]);
